...
from quantization.CapROM_tools import quant_weight_block, adc_quant_error_block
...
class PTQSLQuantLinear(MinMaxQuantLinear):
    ...
    def __init__(self,
                 ...
                 bias_bit=None,
                 bias_correction=False,
                 metric="L2_norm", search_round=1, eq_alpha=0, eq_beta=1, eq_n=100, parallel_eq_n=10, n_H=1, n_V=1,
                 n_a=1, init_layerwise=False):
        super().__init__(in_features, out_features, bias=bias, mode=mode, w_bit=w_bit, a_bit=a_bit, bias_bit=bias_bit,
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None):
        ...
        if metric == "cosine":
            ...
            similarity = F.cosine_similarity(tensor_raw - torch.mean(tensor_raw, dim=-1, keepdim=True),
                                             tensor_sim - torch.mean(tensor_sim, dim=-1, keepdim=True), dim=-1)
        else:
            if metric == "L1_norm":
    ...
    def quant_weight_bias(self):
        ...
        w = (self.weight.view(self.n_V, self.crb_rows, self.n_H, self.crb_cols) / self.w_interval).round_().clamp_(
    ...
    def _search_best_w_interval(self, x, weight_interval_candidates, raw_out_expanded_chunked):
        ...
        for h in range(self.n_H):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                w_sim = self.weight.view(self.n_V, self.crb_rows, self.n_H, self.crb_cols).unsqueeze(
                    0)  # shape: 1,n_V,crb_rows,n_H,crb_cols
                w_sim = (w_sim / cur_w_interval).round_().clamp_(-self.w_qmax, self.w_qmax - 1).mul_(
                    cur_w_interval)  # shape: parallel_eq_n,n_V,crb_rows,n_H,crb_cols
                ...
                out_sim = torch.cat(torch.chunk(out_sim.unsqueeze(-2), chunks=p_ed - p_st, dim=-1),
                                    dim=-2)  # shape: B,*,parallel_eq_n,oc
                out_sim = torch.cat(torch.chunk(out_sim.unsqueeze(-2), chunks=self.n_V, dim=-1),
                                    dim=-2)  # shape: B,*,parallel_eq_n,n_V,crb_rows
                similarity = self._get_similarity(raw_out_expanded_chunked, out_sim,
                                                  self.metric)  # shape: B,*,parallel_eq_n,n_V
                similarity = torch.mean(similarity,
                                        dim=list(range(len(similarity.shape) - 2)))  # shape: parallel_eq_n, n_V
            ...
            tmp_w_interval[:, :, :, h:h + 1, :] = torch.gather(weight_interval_candidates[:, :, :, h:h + 1, :], dim=0,
                                                               index=h_best_index)
    ...
    def _search_best_a_interval(self, x, input_interval_candidates, raw_out_expanded):
        ...
        for a in range(self.n_a):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_sim = (x_sim / (cur_a_interval)).round_().clamp_(-self.a_qmax, self.a_qmax - 1) * (
                    cur_a_interval)  # shape: B,*,n_a,crb_acts,parallel_eq_n
                x_sim = x_sim.permute(*list(range(len(x_sim.shape) - 3)), -1, -3, -2).reshape(*x.shape[:-1],
                                                                                              p_ed - p_st, x.shape[
                                                                                                  -1])  # shape: B,*,parallel_eq_n,ic
                # calculate similarity and store them
                out_sim = F.linear(x_sim, w_sim, bias_sim)  # shape: B,*,parallel_eq_n,oc
            ...
            tmp_a_interval[a:a + 1, :, :] = torch.gather(input_interval_candidates[a:a + 1, :, :], dim=2,
                                                         index=a_best_index)
        self.a_interval = tmp_a_interval.squeeze(-1)

    def _initialize_intervals(self, x):
        if self.init_layerwise:
            self.w_interval = ((self.weight.abs().max()) / (self.w_qmax - 0.5)).view(1, 1, 1, 1).repeat(self.n_V, 1,
                                                                                                        self.n_H, 1)
            self.a_interval = (x.abs().max() / (self.a_qmax - 0.5)).detach().view(1, 1).repeat(self.n_a, 1)
        else:
            self.w_interval = (self.weight.view(self.n_V, self.crb_rows, self.n_H, self.crb_cols).abs().amax([1, 3],
                                                                                                             keepdim=True) / (
                                       self.w_qmax - 0.5))
            self.a_interval = ((x.view(*x.shape[:-1], self.n_a, self.crb_acts).abs().amax(
                list(range(len(x.shape) - 1)) + [-1], keepdim=False)) / (self.a_qmax - 0.5)).unsqueeze(-1)
    ...
    def calibration_step2(self, x):
        ...
        raw_out_expanded_chunked = torch.cat(torch.chunk(raw_out_expanded.unsqueeze(-2), chunks=self.n_V, dim=-1),
                                             dim=-2)  # shape: B,*,1,n_V,crb_rows
        ...
        weight_interval_candidates = torch.tensor(
            [self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n for i in range(self.eq_n + 1)]).cuda().view(
            -1, 1, 1, 1, 1) * self.w_interval.unsqueeze(0)  # shape: eq_n,n_V,1,n_H,1
        input_interval_candidates = torch.tensor(
            [self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n for i in range(self.eq_n + 1)]).cuda().view(
            1, 1, -1) * self.a_interval.unsqueeze(-1)  # shape: n_a,1,eq_n
...
class PostGeluPTQSLQuantLinear(PTQSLQuantLinear):
    def __init__(self,
        ...
        super().__init__(in_features, out_features, bias=bias, mode=mode, w_bit=w_bit, a_bit=a_bit, bias_bit=bias_bit,
                         bias_correction=bias_correction,
                         metric=metric, search_round=search_round, eq_alpha=eq_alpha, eq_beta=eq_beta, eq_n=eq_n,
                         parallel_eq_n=parallel_eq_n, n_H=n_H, n_V=n_V, n_a=n_a, init_layerwise=init_layerwise)
    ...
    def _search_best_a_interval(self, x, input_interval_candidates, raw_out_expanded):
        ...
        for a in range(self.n_a):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                # quantize input
                x_sim = torch.cat(torch.chunk(x.unsqueeze(-2), chunks=self.n_a, dim=-1), dim=-2).unsqueeze(-1)
                x_pos = (x_sim / (cur_a_interval)).round_().clamp_(0, self.a_qmax - 1) * (
                    cur_a_interval)  # shape: B,*,n_a,crb_acts,parallel_eq_n
                x_neg = (x_sim / (self.a_interval[1])).round_().clamp_(-self.a_qmax, 0) * (
                    self.a_interval[1])  # shape: B,*,n_a,crb_acts,1
                x_sim = (x_pos + x_neg).permute(*list(range(len(x_sim.shape) - 3)), -1, -3, -2).reshape(*x.shape[:-1],
                                                                                                        p_ed - p_st,
                                                                                                        x.shape[
                                                                                                            -1])  # shape: B,*,parallel_eq_n,ic
                # calculate similarity and store them
    ...
    def _initialize_intervals(self, x):
        if self.init_layerwise:
            ...
            self.w_interval = (self.weight.view(self.n_V, self.crb_rows, self.n_H, self.crb_cols).abs().amax([1, 3],
                                                                                                             keepdim=True) / (
                                       self.w_qmax - 0.5))
            self.a_interval = [((x.view(*x.shape[:-1], self.n_a, self.crb_acts).amax(
                list(range(len(x.shape) - 1)) + [-1], keepdim=False)) / (self.a_qmax - 0.5)).unsqueeze(-1)]
    ...
    def calibration_step2(self, x):
        ...
        input_interval_candidates = torch.tensor(
            [self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n for i in range(self.eq_n + 1)]).cuda().view(
            1, 1, -1) * self.a_interval[0].unsqueeze(-1)  # shape: n_a,1,eq_n
        for e in range(self.search_round):
            # search for best weight interval
            self._search_best_w_interval(x, weight_interval_candidates, raw_out_expanded_chunked)
            # search for best input interval
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    def __init__(self,
                 ...
                 n_a=1, init_layerwise=False):
        super().__init__(in_features, out_features, bias=bias, mode=mode, w_bit=w_bit, a_bit=a_bit, bias_bit=bias_bit,
                         bias_correction=bias_correction, metric=metric, search_round=search_round, eq_alpha=eq_alpha,
                         eq_beta=eq_beta, eq_n=eq_n, parallel_eq_n=parallel_eq_n, n_H=n_H, n_V=n_V, n_a=n_a,
    ...
    def _initialize_calib_parameters(self):
        ...
        self.calib_batch_size = int(self.raw_input.shape[0] / 8)
        while True:
            numel = (2 * (
                    self.raw_input.numel() + self.raw_out.numel()) / self.calib_size * self.calib_batch_size)  # number of parameters on GPU
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            if self.init_layerwise:
                ...
                a_interval_ = ((x_.view(*x_.shape[:-1], self.n_a, self.crb_acts).abs().amax(
                    list(range(len(x_.shape) - 1)) + [-1], keepdim=False)) / (self.a_qmax - 0.5)).unsqueeze(-1)
    ...
    def _get_pearson_w(self, tensor_raw, tensor_sim):
        ...
        similarity = torch.cosine_similarity(tensor_raw - tensor_raw_mean, tensor_sim - tensor_sim_mean,
                                             dim=1)  # shape: b,n_V,parallel_eq_n
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for h in range(self.n_H):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                raw_out_expanded = torch.cat(torch.chunk(raw_out_expanded.unsqueeze(-2), chunks=self.n_V, dim=-1),
                                             dim=-2)  # shape: b,*,1,n_V,crb_rows
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    if self.metric != "pearson":
                        similarity = self._get_similarity(raw_out_expanded, out_sim, self.metric,
                                                          raw_grad)  # shape: b,*,parallel_eq_n,n_V
                        if len(similarity.shape) > 3:
                            similarity = torch.mean(similarity, dim=list(
                                range(1, len(similarity.shape) - 2)))  # shape: b, parallel_eq_n, n_V
                    else:
        ...
        for a in range(self.n_a):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    if self.metric != "pearson":
                        similarity = self._get_similarity(raw_out_expanded, out_sim, self.metric,
                                                          raw_grad)  # shape: b,*,parallel_eq_n
                        if len(similarity.shape) > 2:
                            similarity = torch.mean(similarity, dim=list(
                                range(1, len(similarity.shape) - 1)))  # shape: b, parallel_eq_n
                    else:
    ...
    def _initialize_intervals(self):
        ...
        if self.init_layerwise:
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
                x_ = self.raw_input[b_st:b_ed].cuda()
                a_interval_ = ((x_.view(*x_.shape[:-1], self.n_a, self.crb_acts).amax(
                    list(range(len(x_.shape) - 1)) + [-1], keepdim=False)) / (self.a_qmax - 0.5)).unsqueeze(-1)
                tmp_a_intervals.append(a_interval_)
            self.a_interval = torch.cat(tmp_a_intervals, dim=1).amax(dim=1, keepdim=True)
        ...
        for a in range(self.n_a):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    x_pos = (x_sim / (cur_a_interval)).round_().clamp_(0, self.a_qmax - 1) * (
                        cur_a_interval)  # shape: b,*,n_a,crb_acts,parallel_eq_n
                    x_neg = (x_sim / (self.a_neg_interval)).round_().clamp_(-self.a_qmax, 0) * (
                        self.a_neg_interval)  # shape: b,*,n_a,crb_acts,1
                    ...
                    similarity = self._get_similarity(raw_out_expanded, out_sim, self.metric,
                                                      raw_grad)  # shape: b,*,parallel_eq_n
                    similarity = torch.mean(similarity,
                                            dim=list(range(1, len(similarity.shape) - 1)))  # shape: b, parallel_eq_n
                    similarity = torch.sum(similarity, dim=0, keepdim=True)  # shape: 1, parallel_eq_n
                    similarities.append(similarity)
                # store best input interval and store in tmp_a_interval
                similarities = torch.cat(similarities, dim=1)  # shape: 1, eq_n
...
class CapROMBatchingQuantLinear(MinMaxQuantLinear):
    def __init__(self,
                 in_features: int,
                 out_features: int,
                 bias: bool = True,
                 ...
                 bias_bit=None,
                 bias_correction=False,
                 metric="L2_norm", search_round=1, eq_alpha=0, eq_beta=1, eq_n=100, parallel_eq_n=10, length_max=7000,
                 delta_length=5, vcm=450, vp=900, vn=0, vref=900, adc_bits=None, w_block=0):
        super().__init__(in_features, out_features, bias=bias, mode=mode, a_bit=a_bit, bias_bit=bias_bit,
                         bias_correction=bias_correction)
        self.metric = metric
        self.search_round = search_round
        self.eq_alpha = eq_alpha
    ...
    def quant_weight_bias(self):
        q_w, pos_sum, neg_sum, cap_length = quant_weight_block(
            self.weight, self.w_block, self.length_max, self.delta_length)

        if self.bias is not None:
            return q_w, pos_sum, neg_sum, self.bias
        else:
            return q_w, pos_sum, neg_sum, None

    def quant_input(self, x):
        x_sim = (x / self.a_interval).round_().clamp_(-self.a_qmax + 1, self.a_qmax - 1).mul_(self.a_interval)
    ...
    def quant_forward(self, x):
        ...
        q_w, pos_sum, neg_sum, bias_sim = self.quant_weight_bias()

        x_sim = self.quant_input(x)

        out_sim = torch.matmul(x_sim, q_w.t())

        error = adc_quant_error_block(out_sim, pos_sum, neg_sum, self.vref, self.adc_bits, self.vp, self.vn,
                                      self.w_block)

        if bias_sim is None:
            out = out_sim + error * self.a_interval
        else:
            out = out_sim + error * self.a_interval + bias_sim

        # if bias_sim is None:
        #     out = out_sim
        # else:
        #     out = out_sim + bias_sim
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            b_ed = min(self.calib_size, b_st + self.calib_batch_size)
            x_ = self.raw_input[b_st:b_ed].cuda()
            a_interval_ = x_.abs().max() / self.a_qmax
            tmp_a_intervals.append(a_interval_)
        self.a_interval = max(tmp_a_intervals)
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None, raw_grad=None):
        ...
        if metric == "cosine":
            ...
            if metric == "L1_norm":
                ...
                raise NotImplementedError(f"metric {metric} not implemented!")
            similarity = similarity.mean(dim=tuple(range(1, similarity.ndim)))
        return similarity
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_shape = x.shape
                x_sim = x.unsqueeze(0).expand(self.parallel_eq_n, *x_shape)

                cur_a_interval = cur_a_interval.unsqueeze(1).unsqueeze(2).unsqueeze(3)

                x_sim = (x_sim / cur_a_interval).round_().clamp_(-self.a_qmax+1, self.a_qmax-1) * cur_a_interval

                out_sim = torch.matmul(x_sim, q_w.t())

                # out_sim_shape = out_sim[0].unsqueeze(0)
                #
                # error = adc_quant_error_block(out_sim_shape, pos_sum, neg_sum, self.vref, self.adc_bits, self.vp, self.vn,
                #                               self.w_block)
                #
                # if bias_sim is None:
                #     out = out_sim + error * cur_a_interval
                # else:
                #     out = out_sim + error * cur_a_interval + bias_sim
                ...
                similarity = self._get_similarity(raw_out_expanded, out, self.metric, raw_grad)
                similarities.append(similarity)

            batch_similarities = torch.cat(similarities, dim=0)

        batch_similarities_avg = batch_similarities.reshape(int(self.calib_size / self.calib_batch_size), -1).mean(
            dim=0)
        best_index = batch_similarities_avg.argmax(dim=0)
        self.a_interval = input_interval_candidates[best_index]
    ...
    def calibration_step2(self):
        ...
        input_interval_candidates = (torch.tensor(
            [self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n for i in range(self.eq_n)]).cuda() *
                                     self.a_interval)
...
class PostGeluCapROMBatchingQuantLinear(CapROMBatchingQuantLinear):
    def __init__(self,
                 in_features: int,
                 out_features: int,
                 bias: bool = True,
        ...
        super().__init__(in_features, out_features, bias=bias, mode=mode, a_bit=a_bit, bias_bit=bias_bit,
                         bias_correction=bias_correction, metric=metric, search_round=search_round, eq_alpha=eq_alpha,
                         eq_beta=eq_beta, eq_n=eq_n, parallel_eq_n=parallel_eq_n, length_max=length_max,
                         delta_length=delta_length, vcm=vcm, vp=vp, vn=vn, vref=vref, adc_bits=adc_bits,
                         w_block=w_block)
        self.a_neg_interval = 0.16997124254703522 / self.a_qmax

    def quant_input(self, x):
        x_pos_sim = (x / self.a_interval).round_().clamp_(0, self.a_qmax - 1).mul_(self.a_interval)
        x_neg_sim = (x / self.a_neg_interval).round_().clamp_(-self.a_qmax + 1, 0).mul_(self.a_neg_interval)

        return (x_pos_sim + x_neg_sim).reshape_as(x)
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            b_ed = min(self.calib_size, b_st + self.calib_batch_size)
            x_ = self.raw_input[b_st:b_ed].cuda()
            a_interval_ = x_.max() / self.a_qmax
            tmp_a_intervals.append(a_interval_)
        self.a_interval = max(tmp_a_intervals)
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_pos_sim = (x_sim / cur_a_interval).round_().clamp_(0, self.a_qmax - 1) * cur_a_interval
                x_neg_sim = (x_sim / self.a_neg_interval).round_().clamp_(-self.a_qmax + 1, 0).mul_(self.a_neg_interval)

                x_sim = (x_pos_sim + x_neg_sim).reshape_as(x_sim)
...
class FPQuantLinear(MinMaxQuantLinear):
    def __init__(self,
                 in_features: int,
                 out_features: int,
                 bias: bool = True,
    ...
    def quant_forward(self, x):
        assert self.calibrated is not None, f"Run calibrate_forward before quant_forward for {self}"
        w_sim, bias_sim = self.quant_weight_bias()
        x_sim = self.quant_input(x)
        out = F.linear(x_sim, w_sim, bias_sim)
        return out
    ...
    def calibration_step2(self):
        ...
        with torch.no_grad():
            ...
            if self.em_mx:
                ...
                self.w_maxval = mse_range_estimator(self.weight, self.n_bits, self.sign_bit, self.mantissa, self.per_channel)
