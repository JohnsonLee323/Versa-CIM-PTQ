...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None, dim=-1):
        ...
        """
        if metric == "cosine":
            similarity = F.cosine_similarity(tensor_raw, tensor_sim,
                                             dim=dim)  # should only support dim=-1 and cannot be paralleled
        elif metric == "pearson":
            similarity = F.cosine_similarity(tensor_raw - torch.mean(tensor_raw), tensor_sim - torch.mean(tensor_sim),
                                             dim=dim)
        else:
            if metric == "L1_norm":
    ...
    def _search_best_A_interval(self, A, B, A_interval_candidates):
        ...
        """
        # recalculate A_pad
        A_pad = F.pad(A, [0, self.pad_cols_A, 0, self.pad_rows_A, 0, self.pad_groups_A]).unsqueeze(0).view(1, -1,
                                                                                                           self.n_G_A,
                                                                                                           self.crb_groups_A,
                                                                                                           self.n_V_A,
                                                                                                           self.crb_rows_A,
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                cur_A_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = A_interval_candidates[p_st:p_ed, :, :, :, v:v + 1,
                                                                     :, h:h + 1, :]
                A_sim = (A_pad / cur_A_interval).round_().clamp_(-self.A_qmax, self.A_qmax - 1).mul_(cur_A_interval)
                A_sim = A_sim.view(p_ed - p_st, -1, A.shape[1] + self.pad_groups_A, A.shape[2] + self.pad_rows_A,
                                   A.shape[
                                       3] + self.pad_cols_A)  # shape: parallel_eq_n,B,H*,dim1*,dim2* (* stand for padding)
                ...
                similarity = similarity.mean(
                    [1, 3])  # shape: parallel_eq_n,H (remaining mean operation will be done later on)
                similarities.append(similarity)
            # calculate best similarity for this block
            similarities = torch.cat(similarities, 0)  # shape: eq_n,H
            similarities = F.pad(similarities, [0, self.pad_groups_A]).view(self.eq_n, self.n_G_A,
                                                                            self.crb_groups_A).mean(
                -1)  # shape: eq_n, n_G_A
            best_index = torch.argmax(similarities, dim=0, keepdim=False).view(1, 1, -1, 1, 1, 1, 1, 1)
            tmp_A_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = torch.gather(
                A_interval_candidates[:, :, :, :, v:v + 1, :, h:h + 1, :], dim=0, index=best_index)
    ...
    def _search_best_B_interval(self, A, B, B_interval_candidates):
        ...
        """
        # recalculate B_pad
        B_pad = F.pad(B, [0, self.pad_cols_B, 0, self.pad_rows_B, 0, self.pad_groups_B]).unsqueeze(0).view(1, -1,
                                                                                                           self.n_G_B,
                                                                                                           self.crb_groups_B,
                                                                                                           self.n_V_B,
                                                                                                           self.crb_rows_B,
        ...
        for v, h in product(range(self.n_V_B), range(self.n_H_B)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                cur_B_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = B_interval_candidates[p_st:p_ed, :, :, :, v:v + 1,
                                                                     :, h:h + 1, :]
                B_sim = (B_pad / cur_B_interval).round_().clamp_(-self.B_qmax, self.B_qmax - 1).mul_(cur_B_interval)
                B_sim = B_sim.view(p_ed - p_st, -1, B.shape[1] + self.pad_groups_B, B.shape[2] + self.pad_rows_B,
                                   B.shape[
                                       3] + self.pad_cols_B)  # shape: parallel_eq_n,B,H*,dim2*,dim3* (* stand for padding)
            ...
            # calculate best similarity for this block
            similarities = torch.cat(similarities, 0)  # shape: eq_n,H
            similarities = F.pad(similarities, [0, self.pad_groups_B]).view(self.eq_n, self.n_G_B,
                                                                            self.crb_groups_B).mean(
                -1)  # shape: eq_n, n_G_B
            best_index = torch.argmax(similarities, dim=0, keepdim=False).view(1, 1, -1, 1, 1, 1, 1, 1)
            tmp_B_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = torch.gather(
                B_interval_candidates[:, :, :, :, v:v + 1, :, h:h + 1, :], dim=0, index=best_index)
    ...
    def _initialize_intervals(self, A, B):
        ...
        A_pad = F.pad(A, [0, self.pad_cols_A, 0, self.pad_rows_A, 0, self.pad_groups_A]).unsqueeze(0).view(1, -1,
                                                                                                           ...
                                                                                                           self.crb_rows_A,
                                                                                                           self.n_H_A,
                                                                                                           self.crb_cols_A)  # shape: 1,B,n_G,crb_groups,n_V,crb_rows,n_H,crb_cols
        B_pad = F.pad(B, [0, self.pad_cols_B, 0, self.pad_rows_B, 0, self.pad_groups_B]).unsqueeze(0).view(1, -1,
                                                                                                           self.n_G_B,
                                                                                                           self.crb_groups_B,
                                                                                                           self.n_V_B,
        ...
        if self.init_layerwise:
            self.A_interval = (A.abs().max() / (self.A_qmax - 0.5)).detach().view(1, 1, 1, 1, 1, 1, 1).repeat(1,
                                                                                                              self.n_G_A,
                                                                                                              1,
                                                                                                              self.n_V_A,
                                                                                                              1,
                                                                                                              self.n_H_A,
                                                                                                              1)
            self.B_interval = (B.abs().max() / (self.B_qmax - 0.5)).detach().view(1, 1, 1, 1, 1, 1, 1).repeat(1,
                                                                                                              self.n_G_B,
                                                                                                              1,
                                                                                                              self.n_V_B,
                                                                                                              1,
            ...
            self.A_interval = (A_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.A_qmax - 0.5)).detach().squeeze(
                0)  # shape: 1,n_G,1,n_V,1,n_H,1
            self.B_interval = (B_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.B_qmax - 0.5)).detach().squeeze(
                0)  # shape: 1,n_G,1,n_V,1,n_H,1
    ...
    def calibration_step2(self, A, B):
        ...
        A_interval_candidates = torch.tensor(
            ...
            -1, 1, 1, 1, 1, 1, 1, 1) * self.A_interval.unsqueeze(0)
        B_interval_candidates = torch.tensor(
            [self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n for i in range(self.eq_n + 1)]).cuda().view(
            -1, 1, 1, 1, 1, 1, 1, 1) * self.B_interval.unsqueeze(0)
...
class SoSPTQSLQuantMatMul(PTQSLQuantMatMul):
    ...
    def __init__(self, A_bit=8, B_bit=8, mode="raw",
        ...
        super().__init__(A_bit=A_bit, B_bit=B_bit, mode=mode,
                         ...
                         n_G_A=n_G_A, n_V_A=n_V_A, n_H_A=n_H_A, n_G_B=n_G_B, n_V_B=n_V_B, n_H_B=n_H_B,
    ...
    def _search_best_A_interval(self, A, B, split_candidates):
        ...
        for i in range(len(split_candidates)):
            ...
            A_high = (A_.clamp(split_candidates[i], 1) * (self.A_qmax - 1)).round_().clamp_(0, self.A_qmax - 1) / (
                        self.A_qmax - 1)
            A_low = (A_.clamp(0, split_candidates[i]) / cur_A_interval).round_().clamp_(0,
                                                                                        self.A_qmax - 1) * cur_A_interval
...
class PTQSLBatchingQuantMatMul(PTQSLQuantMatMul):
    def __init__(self, A_bit=8, B_bit=8, mode="raw",
                 metric="L2_norm", search_round=1, eq_alpha=0.1, eq_beta=2, eq_n=100, parallel_eq_n=10,
                 n_G_A=1, n_V_A=1, n_H_A=1, n_G_B=1, n_V_B=1, n_H_B=1, init_layerwise=False):
        super().__init__(A_bit=A_bit, B_bit=B_bit, mode=mode, metric=metric, search_round=search_round,
                         eq_alpha=eq_alpha, eq_beta=eq_beta, eq_n=eq_n, parallel_eq_n=parallel_eq_n, n_G_A=n_G_A,
                         n_V_A=n_V_A, n_H_A=n_H_A, n_G_B=n_G_B, n_V_B=n_V_B, n_H_B=n_H_B, init_layerwise=init_layerwise)
    ...
    def _initialize_calib_parameters(self):
        ...
        while True:
            numel = ((self.raw_input[0].numel() + self.raw_input[
                1].numel() + 2 * self.raw_out.numel()) / self.calib_size * self.calib_batch_size)  # number of parameters on GPU
    ...
    def _initialize_intervals(self):
        # pad A and B for future quantization
        self._get_padding_parameters(self.raw_input[0],
                                     self.raw_input[1])  # put it here because hessian does not use calibration step 1
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            if self.init_layerwise:
                A_interval = (A.abs().max() / (self.A_qmax - 0.5)).detach().view(1, 1, 1, 1, 1, 1, 1).repeat(1,
                                                                                                             self.n_G_A,
                                                                                                             1,
                                                                                                             self.n_V_A,
                                                                                                             1,
                                                                                                             self.n_H_A,
                                                                                                             1)
                B_interval = (B.abs().max() / (self.B_qmax - 0.5)).detach().view(1, 1, 1, 1, 1, 1, 1).repeat(1,
                                                                                                             self.n_G_B,
                                                                                                             1,
                                                                                                             self.n_V_B,
                                                                                                             1,
                ...
                A_pad = F.pad(A, [0, self.pad_cols_A, 0, self.pad_rows_A, 0, self.pad_groups_A]).unsqueeze(0).view(1,
                                                                                                                   -1,
                                                                                                                   self.n_G_A,
                                                                                                                   self.crb_groups_A,
                                                                                                                   self.n_V_A,
                                                                                                                   ...
                                                                                                                   self.n_H_A,
                                                                                                                   self.crb_cols_A)
                B_pad = F.pad(B, [0, self.pad_cols_B, 0, self.pad_rows_B, 0, self.pad_groups_B]).unsqueeze(0).view(1,
                                                                                                                   -1,
                                                                                                                   self.n_G_B,
                                                                                                                   self.crb_groups_B,
                                                                                                                   self.n_V_B,
                                                                                                                   ...
                                                                                                                   self.n_H_B,
                                                                                                                   self.crb_cols_B)
                A_interval = (A_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.A_qmax - 0.5)).detach().squeeze(
                    0)  # shape: 1,n_G,1,n_V,1,n_H,1
                B_interval = (B_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.B_qmax - 0.5)).detach().squeeze(
                    0)  # shape: 1,n_G,1,n_V,1,n_H,1
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None, dim=-1, raw_grad=None):
        ...
        if metric == "cosine":
            ...
            similarity = F.cosine_similarity(tensor_raw - torch.mean(tensor_raw, dim=dim, keepdim=True),
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    # quantize A
                    cur_A_interval = tmp_A_interval.repeat(p_ed - p_st, 1, 1, 1, 1, 1, 1, 1)
                    cur_A_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = A_interval_candidates[p_st:p_ed, :, :, :,
                                                                         v:v + 1, :, h:h + 1, :]
                    A_sim = (A_pad / cur_A_interval).round_().clamp_(-self.A_qmax, self.A_qmax - 1).mul_(cur_A_interval)
                    A_sim = A_sim.view(p_ed - p_st, -1, A.shape[1] + self.pad_groups_A, A.shape[2] + self.pad_rows_A,
                                       A.shape[
                    ...
                    similarity = self._get_similarity(raw_out, out_sim, self.metric,
                                                      raw_grad=raw_grad)  # shape: parallel_eq_n,b,H,dim1
                    similarity = similarity.mean(
                        [3])  # shape: parallel_eq_n,b,H (remaining mean operation will be done later on)
            ...
            batch_similarities = F.pad(batch_similarities, [0, self.pad_groups_A]).view(self.eq_n, self.n_G_A,
                                                                                        self.crb_groups_A).mean(
                -1)  # shape: eq_n, n_G_A
        ...
        for v, h in product(range(self.n_V_B), range(self.n_H_B)):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    # quantize B
                    cur_B_interval = tmp_B_interval.repeat(p_ed - p_st, 1, 1, 1, 1, 1, 1, 1)
                    cur_B_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = B_interval_candidates[p_st:p_ed, :, :, :,
                                                                         v:v + 1, :, h:h + 1, :]
                    B_sim = (B_pad / cur_B_interval).round_().clamp_(-self.B_qmax, self.B_qmax - 1).mul_(cur_B_interval)
                    B_sim = B_sim.view(p_ed - p_st, -1, B.shape[1] + self.pad_groups_B, B.shape[2] + self.pad_rows_B,
                                       B.shape[
                ...
                batch_similarities.append(similarities)
            batch_similarities = torch.cat(batch_similarities, dim=1).sum(dim=1, keepdim=False)  # shape: eq_n,H
            batch_similarities = F.pad(batch_similarities, [0, self.pad_groups_B]).view(self.eq_n, self.n_G_B,
                                                                                        self.crb_groups_B).mean(
                -1)  # shape: eq_n, n_G_B
            best_index = torch.argmax(batch_similarities, dim=0, keepdim=False).view(1, 1, -1, 1, 1, 1, 1, 1)
            tmp_B_interval[:, :, :, :, v:v + 1, :, h:h + 1, :] = torch.gather(
    ...
    def _search_best_A_interval(self, split_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for i in range(len(split_candidates)):
                # quantize A
                cur_A_interval = split_candidates[i] / (self.A_qmax - 1)
                A_high = (A.clamp(split_candidates[i], 1) * (self.A_qmax - 1)).round_().clamp_(0, self.A_qmax - 1) / (
                            self.A_qmax - 1)
                A_low = (A.clamp(0, split_candidates[i]) / cur_A_interval).round_().clamp_(0,
                                                                                           self.A_qmax - 1) * cur_A_interval
                A_sim = A_high + A_low  # shape: 1,b,H,S,S
                # quantize B, this quantization is optimized out of loop
                # calculate similarity and store them (dim1=dim2=S, dim3=W)
...
class FPQuantMatMul(MinMaxQuantMatMul):
    def __init__(self,
                 mode = "raw",
                 n_bits = 8,
                 sign_bit = 1,
    ...
    def quant_input_A(self, x):
        x = quantize_to_fp(x, self.n_bits, self.A_maxval, self.mantissa, self.sign_bit)
        return x

    def quant_input_B(self, x):
        x = quantize_to_fp(x, self.n_bits, self.B_maxval, self.mantissa, self.sign_bit)
        return x
    ...
    def calibration_step2(self):
        ...
        with torch.no_grad():
            ...
            raw_input_A = self.raw_input[0].to('cuda:0')

            raw_input_B = self.raw_input[1].to('cuda:0')

            if self.em_mx:

                self.raw_out = self.raw_out.to('cuda:0')
                ...
                for mb in range(1, self.n_bits):
                    ...
                    current_mantissa = torch.tensor([mb], dtype=torch.float32).to('cuda:0')

                    self.mantissa = current_mantissa

                    self.A_maxval = mse_range_estimator(raw_input_A, self.n_bits, self.sign_bit, current_mantissa)

                    self.B_maxval = mse_range_estimator(raw_input_B, self.n_bits, self.sign_bit, current_mantissa)

                    A_sim = quantize_to_fp(raw_input_A, self.n_bits, self.A_maxval, current_mantissa, self.sign_bit)

                    B_sim = quantize_to_fp(raw_input_B, self.n_bits, self.A_maxval, current_mantissa, self.sign_bit)

                    out = (A_sim @ B_sim).cuda()
                ...
                self.A_maxval = mse_range_estimator(raw_input_A, self.n_bits, self.sign_bit, self.mantissa)

                self.B_maxval = mse_range_estimator(raw_input_B, self.n_bits, self.sign_bit, self.mantissa)
